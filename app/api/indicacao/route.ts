...
export async function POST(req: NextRequest) {
  ...
  if (novo.perfil_ia?.indicado_por) return NextResponse.json({ ok: false, motivo: 'ja_usado' })
  ...
  const { data: refRows } = await admin.from('progresso').select('user_id, moedas, perfil_ia').eq('user_id', ref).limit(1)
  const indicador = refRows?.[0]
  if (!indicador) return NextResponse.json({ ok: false, motivo: 'indicador_nao_encontrado' })

  // 5) Recompensa o NOVO aluno: +2 dias de trial Premium (a partir de agora ou do fim do trial atual).
  ...
  // 6) Recompensa o INDICADOR: +100 moedas e contador de indicações.
  const perfilInd = indicador.perfil_ia || {}
  await admin.from('progresso').update({
    moedas: (indicador.moedas || 0) + 100,
